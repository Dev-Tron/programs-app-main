  ...
  <main>
    <div className='p-4 md:p-6 mt-2 md:mt-3 lg:mt-8'>
      <div className='flex gap-4'>
        ...
        <input className='bg-transparent w-9/12 md:text-2xl focus:border-b-2 outline-0 focus:border-light-grey pb-3 w-full caret-dark-red' type='text' placeholder='Search for bookmarked shows' onChange={event => {setSearchProgram(event.target.value)}}/>
      </div>
      <section>
        <div className='mt-5 md:mt-10'>
        <h1 className='text-xl md:text-2.5xl text-white'>Bookmarked Movies</h1>
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6 md:mt-4'>
          {data.filter((val => {
            ...
            return (
              <Fragment key={i}>
                { program.isBookmarked && program.category === "Movie" &&
                <div className='mt-4 relative group'>
                  ...
                  <div className='mt-2'>
                    <div className='flex text-xs md:text-base gap-2'>
                      <p>{program.year}</p>
                      <span>&#8226;</span>
                      <Image src="/images/icon-category-movie.svg" alt='movie' width={20} height={20}/>
      ...
      <section>
        <div className='mt-5 md:mt-10'>
          <h1 className='text-xl md:text-2.5xl text-white'>Bookmarked TV Series</h1>
          <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6 md:mt-4'>
            {data.filter((val => {
              if (searchProgram === '') {
                return val
              ...
              return (
                <Fragment key={i}>
                  { program.isBookmarked && program.category === "TV Series" &&
                  <div className='mt-4 relative group'>
                    <div className='hover:brightness-50 transition ease-out delay-150'>
                      <Image className='md:hidden rounded-lg' src={program.thumbnail.regular?.small} alt={program.title}/>
                      <Image className='xs:hidden rounded-lg' src={program.thumbnail.regular?.large} alt={program.title}/>
